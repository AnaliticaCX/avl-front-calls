...
interface ConnectedCallFilters {
    conn_id: string;
    customer_id: string;
    telephone: string;
    start_date: string;
...
export default function ConnectedCallsPage() {
    const [showNote, setShowNote] = useLocalStorage('calls-connected-hide-note', false);
    const [dateRangeError, setDateRangeError] = useState("");
    const router = useRouter();
    const { results, loading, error, search, setError } = useSearch('/api/calls/detalle_llamadas/search');
    const { filters, updateFilter, clearFilters } = useFilters<ConnectedCallFilters>({
        conn_id: '',
        customer_id: '',
        telephone: '',
        start_date: '',
    ...
    const columns = [
        {
            ...
            render: (_: any, row: any) => (
                <div>
                    <div className="text-sm font-medium text-gray-900">{row.agent_name || '-'}</div>
                    <div className="text-xs text-gray-500">ID: {row.agent_id || '-'}</div>
                </div>
            )
        },
        {
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <Breadcrumbs items={[
                ...
                { label: 'Detalle Llamadas', href: '/calls/detalle_llamadas' }
            ]} />

            <PageHeader
                title="Detalle de Llamadas Conectadas"
                description="Consulta el detalle de llamadas conectadas."
            />
                ...
                <div>
                    ...
                    <div className="mt-6 flex flex-col items-center gap-2">
                        ...
                        <div className="text-sm text-gray-500">
                            Mostrando {results.data.length} de {results.total} llamadas conectadas
                        </div>
                    </div>
                </div>
            )}
