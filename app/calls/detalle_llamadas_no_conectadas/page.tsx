...
export default function NotConnectedCallsPage() {
    const [showNote, setShowNote] = useLocalStorage('calls-notconnected-hide-note', false);
    const [dateRangeError, setDateRangeError] = useState("");
    const router = useRouter();
    const { results, loading, error, search, setError } = useSearch('/api/calls/detalle_llamadas_no_conectadas/search');
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <Breadcrumbs items={[
                ...
                { label: 'Detalle Llamadas No Conectadas', href: '/calls/detalle_llamadas_no_conectadas' }
            ]} />
                ...
                <div>
                    ...
                    <div className="mt-6 flex flex-col items-center gap-2">
                        ...
                        <div className="text-sm text-gray-500">
                            Mostrando {results.data.length} de {results.total} llamadas no conectadas
                        </div>
                    </div>
                </div>
            )}
            ...
            {!loading && results && results.data && results.data.length === 0 && (
                <EmptyState
                    ...
                    message="No hay llamadas no conectadas que coincidan con los criterios de búsqueda."
                />
            )}
        </div>
    );
